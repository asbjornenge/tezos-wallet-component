...
export const TezosWallet = (props) => {
  const [tezIDProfile, setTezIDProfile] = useState(null)

  useEffect(() => {
   if (props.tezidProfile && props.address) {
      (async () => {
        try {
          const profile = await fetch(`https://tezid.net/api/${props.network}/avatar/${props.address}`).then(r => r.json())
          setTezIDProfile(profile)
        } catch(e) {
          console.error(`Unable to get TezID Profile for ${props.address}`)
        }
      })()
    } 
  ...
  if (props.tezidProfile && tezIDProfile && tezIDProfile?.avatar ) {
    idimgStyle = { backgroundImage: `url(${getIpfsLink(tezIDProfile.avatar)})` }
  }
